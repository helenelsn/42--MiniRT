...
bool	is_directly_illuminated(t_app *app, t_light *light, \
	t_ray ray, t_ray *obj_to_light)
{
	double	obj_to_light_dist;

	obj_to_light->origin = ray.hit_info.hit_point;
	obj_to_light->direction = get_directional_vect(ray.hit_info.hit_point, \
		light->p);
	obj_to_light_dist = obj_to_light->direction.norm;
	normalise(&obj_to_light->direction);
	no_tree_intersections(app, obj_to_light, get_interval(HITPOINT_OFFSET, \
		obj_to_light_dist));
...
t_color	compute_lighting(t_app *app, t_ray ray)
{
	...
	effective_color = color_mult(app->p_data.mooooo->infos.emitted_color, \
		ray.hit_info.color);
	while (light)
	{
