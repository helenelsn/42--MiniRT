...
/*   Updated: 2023/12/17 01:15:30 by hlesny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	get_rotation_angle(t_vec surface_normal, t_vec unit_vector)
{
	...
	dot_product = surface_normal.x * unit_vector.x + surface_normal.y
		* unit_vector.y + surface_normal.z * unit_vector.z;
		...
		t_point obj_p)
{
	...
	if (!ft_is_equalsf(surface_normal.y, 1.f, FLT_EPSILON))
	{
		...
		position = change_base_of_vec(position, (t_vec){0, 0, 0, 0},
				rotation_mat);
