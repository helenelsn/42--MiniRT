...
/*   Updated: 2023/12/16 22:42:01 by hlesny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sphere *create_sphere(char **tab,  t_parsing_data *data)
{
    ...
    init_mat(&mat);
    if (!get_rgb(tab[3], &mat.color) || !ft_strisfloat(tab[2]) ||elem->radius < 0
        || !get_point(tab[1], &elem->p) || !set_bonus_options(&mat, tab + SP_PL_ARG_SIZE))
        {
            free(elem);
            return NULL;
        }
